...
| expreg | ported/partial | repeated `SPC v` expands lexical subword → symbol → Python/JSON syntax nodes, with whitespace-sensitive balanced-list interiors and enclosing-pair tiers both in code and inside parser-bounded ordinary/block strings, plus a paragraph fallback in parserless modes (`src/vi.lisp`, `scripts/expreg-test.sh`). `M-x expreg-contract`, declared but unbound in the Emacs configuration, walks backward through the generated buffer-local sequence; `SPC v` then moves forward again. Parser results and contraction state are valid only for the unchanged buffer. Other languages retain textual fallback; malformed error fragments use useful containing nodes rather than Expreg's partial error ranges, and a sequence begun from an arbitrary Visual selection does not reconstruct smaller generated candidates for contraction. |
