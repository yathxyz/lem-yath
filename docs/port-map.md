...
| evil-org | ported/partial | native `.org` buffers provide mode-local `Tab`/`Shift-Tab` visibility cycling, visible-line `j/k`, `gh/gl/gk/gj/gH` heading navigation, Org-aware `o/O`, and context-dispatched Meta editing. `M-h/l` targets one heading/list item/table column or a prose word; `M-k/j` moves heading/list trees or table rows; `M-H/L` uses subtree/list-tree scope or table columns; `M-K/J` deletes/inserts a table row or drags one literal non-CLOCK line. Ordered/structurally tabbed list transforms, point-only continuation indentation, formula-table structure edits, CLOCK dragging, and visual Meta operations fail closed. Source-block lookalikes are excluded from heading/list/table dispatch, though literal line dragging remains available. Normal `t/T`, `Return`, and `M-o` retain Evil-Snipe/Evil/window ownership. Text objects, true `<`/`>` ranges, region-aware Meta behavior, broader element navigation/endpoints, and shift-control/calendar semantics remain gaps (`src/org/`, `scripts/org-test.sh`). |
