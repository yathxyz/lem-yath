...
| consult | ported/partial | `src/project.lisp` supplies persistent project MRU, tracked+untracked annotated file selection, annotated project-buffer membership by directory, bounded asynchronous regexp search, and Emacs-style switch dispatch on `SPC p f/g/p` and `SPC SPC`. The switch menu preserves `f/g/d/v/e/o`: `v` opens Git through Legit at the selected root, while `e` uses Lem's rooted terminal and `o` uses rooted M-x-style command execution, approximations of `project-eshell` and `project-any-command`. Preview-on-move and `SPC SPC`'s combined Project Buffer/File/Root groups remain gaps. |
