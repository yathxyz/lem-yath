...
- **Git**: magit (`SPC g g/G`) + git-gutter + git-timemachine; smart jj/git dispatch; the focused Lem jj porcelain now covers row-aware describe/new/whole-change-squash/edit/undo/redo/confirmed-abandon/diff workflows, while Majutsu's split, arbitrary or partial squash, rebase, bookmark, conflict, and partial-patch surfaces remain approximate.
