...
- **Git**: magit (`SPC g g/G`) + git-gutter + git-timemachine; smart jj/git dispatch; the focused Lem jj porcelain now covers row-aware describe/new/whole-change-squash/selected-row-rebase/Majutsu-style duplicate/local-bookmark/edit/undo/redo/confirmed-abandon/diff workflows, while Majutsu's split, arbitrary or partial squash, multi-selection rebase or duplicate, configurable duplicate descriptions, remote/multi-bookmark, conflict, and partial-patch surfaces remain approximate.
