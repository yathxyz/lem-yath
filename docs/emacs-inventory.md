...
- The **gptel backends** include bounded Claude/Codex/Grok process adapters with native event rendering and resumable per-backend sessions plus credential-safe Perplexity and GitHub Copilot Chat HTTP adapters. Org-tree conversation forking, per-request backend controls, richer Copilot request families, and ChatGPT Codex/Grok native OAuth remain gaps.
